...
if TYPE_CHECKING:
    ...
    HexColor = str          # see TypeError in match_hexcolor()
    HexColorRange = str     # see TypeError in match_hexcolor_range()
...
def translateRgb(rgbcolor: HexColor | TupColor, dstnchannel: Literal[1,3,4] = None) -> np.ndarray:
    ...
    if dstnchannel and dstnchannel != len(rgbcolor):
        if dstnchannel == 1:
            rgbcolor = (int(rgb2gray(*rgbcolor[:3]) + 0.5),)
...
def isColorsInRgbrange(immat: np.ndarray, rgbrange: HexColorRange):
